...
TEST_CASE("not_null<T>", "[concepts]") {
  SECTION("T is trivially_copyable") {
    SECTION("not_null<T> is trivially_copyable") {
      STATIC_REQUIRE(std::is_trivially_copyable_v<not_null<int*>>);
    }
  }
  SECTION("T is pointer_like") {
    SECTION("not_null<T> is pointer_like") {
      STATIC_REQUIRE(pointer_like<not_null<int*>>);
    }
  }
  SECTION("T is traversable_pointer") {
    SECTION("not_null<T> is traversable_pointer") {
      STATIC_REQUIRE(traversable_pointer<not_null<int*>>);
    }
  }
}
...
TEST_CASE("not_null<T>::rebind<U>", "[template]") {
  SECTION("T is a raw pointer") {
    using sut_type = not_null<int*>;
    using result_type = typename sut_type::template rebind<void>;

    STATIC_REQUIRE(std::is_same_v<result_type,not_null<void*>>);
  }
}
